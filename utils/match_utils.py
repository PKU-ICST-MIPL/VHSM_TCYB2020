...
def context_layer(in_question_repres, in_passage_repres,
     ...
     question_context_representation = tf.concat(axis=2,values=[question_context_representation_fw, question_context_representation_bw])
     passage_context_representation = tf.concat(axis=2,values=[passage_context_representation_fw, passage_context_representation_bw])
     return (question_context_representation,passage_context_representation)


def context_layer_image(in_question_repres,question_lengths,question_mask,input_dim,is_training,options=None):
     with tf.variable_scope('context_layer'):
        for i in xrange(options.context_layer_num): # support multiple context layer
            with tf.variable_scope('layer-{}'.format(i)):
                # contextual lstm for both passage and question
                ...
                (question_context_representation_fw, question_context_representation_bw,
                 in_question_repres) = layer_utils.my_lstm_layer(
                        ...
                        reuse=False, is_training=is_training, dropout_rate=options.dropout_rate, use_cudnn=options.use_cudnn)
     question_context_representation = tf.concat(axis=2,values=[question_context_representation_fw, question_context_representation_bw])
     return question_context_representation
